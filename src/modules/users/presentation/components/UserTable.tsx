    ...
    <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 overflow-x-auto shadow-2xl">
      <table className="w-full text-left border-collapse">
        <thead className="bg-zinc-950/50 border-b border-zinc-800">
          <tr>
            <th className="p-4 text-xs font-bold text-zinc-400 uppercase tracking-widest pl-8">Usuario</th>
            <th className="p-4 text-xs font-bold text-zinc-400 uppercase tracking-widest">Email</th>
            <th className="p-4 text-xs font-bold text-zinc-400 uppercase tracking-widest">Rol</th>
            <th className="p-4 text-xs font-bold text-zinc-400 uppercase tracking-widest">Estado</th>
            <th className="p-4 text-xs font-bold text-zinc-400 uppercase tracking-widest pr-8">Acciones</th>
          </tr>
        </thead>
          ...
          {isLoading ? (
            <tr>
              <td colSpan={5} className="p-12 text-center">
                ...
                <p className="text-sm text-zinc-500 font-medium uppercase tracking-widest">Cargando equipo...</p>
              </td>
            </tr>
          ) : users.length === 0 ? (
            <tr>
              <td colSpan={5} className="p-12 text-center">
                <p className="text-sm text-zinc-650 font-medium uppercase tracking-widest">No hay otros miembros en el equipo</p>
              </td>
            </tr>
          ) : (
            users.map((u) => (
              ...
              >
                <td className="p-4 pl-8">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[15px] font-semibold text-white">{u.firstName} {u.lastName}</p>
                      <p className="text-xs text-sky-400/90 font-medium mt-0.5">Miembro desde {new Date(u.createdAt).toLocaleDateString()}</p>
                    </div>
                  </div>
                </td>
                <td className="p-4">
