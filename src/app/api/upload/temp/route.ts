...
export async function POST(req: NextRequest) {
  try {
    ...
    // Creamos un nuevo objeto File con el buffer optimizado para reutilizar StorageService
    const optimizedFile = new File([new Uint8Array(optimizedBuffer)], "optimized.webp", { type: "image/webp" });
    
    const storageService = new LocalStorageService();
    const tempUrl = await storageService.upload(optimizedFile, { isTemp: true });

    return NextResponse.json({ url: tempUrl }, { status: 201 });
