...
async function main() {
  console.log('🔍 Iniciando auditoría comparativa de clientes con el archivo raíz clientes-woocommerce - clientes-woocommerce.csv.csv...');

  // 1. Cargar archivo CSV de la raíz
  const filePath = path.join(process.cwd(), 'clientes-woocommerce - clientes-woocommerce.csv.csv');
  if (!fs.existsSync(filePath)) {
    console.error(`❌ Archivo CSV no encontrado en la raíz: ${filePath}`);
    process.exit(1);
  }

  // xlsx puede leer archivos CSV directamente
  const workbook = XLSX.readFile(filePath);
  const sheetName = workbook.SheetNames[0];
  const sheet = workbook.Sheets[sheetName];
  const rows = XLSX.utils.sheet_to_json(sheet) as Record<string, any>[];
  console.log(`📊 Se leyeron ${rows.length} filas del archivo CSV de la raíz.`);
  ...
  console.log(`\n==================================================`);
  console.log(`📊 RESULTADOS DE LA COMPARACIÓN CON CSV RAÍZ`);
  console.log(`==================================================`);
  console.log(`Clientes totales en CSV:      ${rows.length}`);
  ...
  const reportPath = path.join(
    ...
    '4c2dde51-4094-4bcf-8993-e07d21eddbb6', 'auditoria_clientes_raiz.md'
  );

  let mdContent = `# 📊 Reporte de Auditoría: Comparación de Clientes (CSV Raíz)\n\n`;
  mdContent += `Generado el: ${new Date().toLocaleString()}\n`;
  mdContent += `Origen de datos: \`clientes-woocommerce - clientes-woocommerce.csv.csv\` vs Base de Datos Postgres (Prisma)\n\n`;

  mdContent += `## 📈 Resumen General\n\n`;
  mdContent += `| Métrica | Cantidad | Descripción |\n`;
  mdContent += `| :--- | :---: | :--- |\n`;
  mdContent += `| **Clientes Totales en CSV** | ${rows.length} | Cantidad de registros cargados desde el archivo WooCommerce de la raíz |\n`;
  ...
  mdContent += `| **Diferencias de Vendedor** | ${repMismatches.length} | Clientes cuyo vendedor asignado en la DB no coincide con el CSV |\n`;
  mdContent += `| **Diferencias de Estado** | ${activeStatusMismatches.length} | Clientes cuyo estado de aprobación difiere entre CSV y DB |\n\n`;

  if (missingUsers.length > 0) {
    mdContent += `## ❌ Clientes Faltantes en la Base de Datos (${missingUsers.length})\n\n`;
    mdContent += `Estos clientes existen en el CSV de WooCommerce de la raíz pero no tienen una cuenta en la base de datos actual:\n\n`;
    ...
    mdContent += `## ❌ Clientes Faltantes en la Base de Datos\n\n¡Todos los clientes del CSV de la raíz están presentes en la base de datos! 🎉\n\n`;
  }

  if (repMismatches.length > 0) {
    mdContent += `## ⚠️ Diferencias de Vendedor Asignado (${repMismatches.length})\n\n`;
    mdContent += `Clientes existentes cuyas asignaciones de vendedor no coinciden con el CSV original:\n\n`;
    mdContent += `| Cliente / Razón Social | RUT | Email | Vendedor en CSV | Vendedor en DB |\n`;
    mdContent += `| :--- | :--- | :--- | :--- | :--- |\n`;
  ...
  if (activeStatusMismatches.length > 0) {
    ...
    mdContent += `| Nombre | Email | Aprobado en CSV | Activo en DB |\n`;
    mdContent += `| :--- | :--- | :---: | :---: |\n`;
